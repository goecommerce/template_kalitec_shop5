{block name='layout-header-shop-nav-account'}
    {navitemdropdown tag="li"
        ...
        no-caret=true
        right=true
        text="{if JTL\Session\Frontend::getCustomer()->getID() === 0}<span class=\"fas fa-user\"></span><span class=\"d-none d-lg-inline text-uppercase fw-bold\">&nbsp;{lang key="login"}</span>{else}<span class=\"fas fa-user-check\"></span><span class=\"d-none d-lg-inline text-uppercase fw-bold\">&nbsp;{lang key="myAccount"}</span>{/if}"
